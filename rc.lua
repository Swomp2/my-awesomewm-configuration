...
beautiful.init("/home/swomp/.config/awesome/theme.lua")
...
keyboardprefix = wibox.widget {
    text = "|  ",
    font = "Font Awesome 10",
    widget = wibox.widget.textbox(),
}

brightnessprefix = wibox.widget {
    text = " |  ",
    font = "Font Awesome 10",
    widget = wibox.widget.textbox(),
}
...
brightnesswidget:set_text("check your /sys/class/backlight dir")
gears.timer {
    timeout   = 0.05,
    call_now  = true,
    autostart = true,
    callback  = function()
        if file_exists("/sys/class/backlight/amdgpu_bl0/actual_brightness") then
            local brightnessfile = io.open("/sys/class/backlight/amdgpu_bl0/actual_brightness", "r")
            local content = tonumber(brightnessfile:read "*a") // 2.6
            if content > 95 then
                content = 100
            end
            brightnesswidget:set_text(content)
...
batterywidget:set_text("check your /sys/class/power_supply dir")
gears.timer {
    timeout   = 10,
    call_now  = true,
    autostart = true,
    callback  = function()
        if file_exists("/sys/class/power_supply/BAT1/capacity") then
            local batteryfile = io.open("/sys/class/power_supply/BAT1/capacity", "r")
            local content = batteryfile:read "*a"
            batterywidget:set_text(content)
            batteryfile:close()
        end
    end
}
...
dataprefix = wibox.widget {
    text = "% |  ",
    font = "Font Awesome 10",
    widget = wibox.widget.textbox(),
}

clockprefix = wibox.widget {
    text = " |  ",
    font = "Font Awesome 10",
    widget = wibox.widget.textbox(),
}

-- Create a data widget
mydata = wibox.widget {
    format = "%Y-%m-%d",
    ...
    s.mytaglist = awful.widget.taglist {
        ...
        widget_template = {
            ...
            create_callback = function(self, c3, index, objects)
                ...
                if focused then
                    self:get_children_by_id("underline")[1].bg = "#8fbcbb"
                else
                    self:get_children_by_id("underline")[1].bg = "transparent"
                end
            end,
...
globalkeys = gears.table.join(
    ...
    -- Screenshot
    awful.key({}, "Print", function () awful.spawn.with_shell("scrot -q 100 -f ~/Pictures/$(date +'%Y%m%d%H%M%S')_screenshot.png && notify-send 'Скриншот сделан)'") end,
